...
fun EncryptionUi(modifier: Modifier = Modifier) {
    ...
    val key =  encryptionKey.toIntOrNull() ?: 0
    var isKeyError by remember { mutableStateOf(false) }
    val result = caesarCipherEncryption(text, key)
    ...
    Column(
        ...
        if (showResult.value && text.isNotEmpty() && key in 1..26) {
            Text(text = "Encrypted Text: $result",
...
fun isValidKey(key: String): Boolean {
    val keyInt = key.toIntOrNull()
    return keyInt != null && keyInt in 0..27
}
